...
export default class TestAdaptive extends cc.Component {
    ...
    onListRender(item: any, idx: number) {
        let avatarNode: any = item.getChildByName('avatarNode');
        let avatar: any = avatarNode.getComponentInChildren(cc.Sprite);
        let timeNode: any = item.getChildByName('timeNode');
        let time: any = timeNode.getComponentInChildren(cc.Label);
        let chatBg: any = item.getChildByName('chatBg').getComponent(cc.Sprite);
        let chatBgLayout: any = chatBg.node.getComponent(cc.Layout);
        let richtext: any = chatBg.node.getComponentInChildren(cc.RichText);
